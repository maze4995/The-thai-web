...
export function TherapistColumn({ therapist, workDate, onAddSlot, onEditSlot, onDropSlot, onSwapSlots, onDropColumn }: Props) {
  ...
  const nameInitial = therapist.name.charAt(0)
  ...
  return (
    <div
      ...
      className="relative flex flex-col bg-[#161b27] border border-slate-700/40 rounded-xl overflow-hidden"
    >
      {/* Left drop indicator */}
      {showLeftIndicator && (
      ...
      <div
        ...
        className="px-3 py-2.5 bg-[#1a2035] border-b border-slate-700/40 flex items-center justify-between cursor-grab active:cursor-grabbing select-none"
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <span className="font-bold text-white text-[16px] leading-tight">{therapist.name}</span>
            {isServing && (
              <span className="text-[10px] font-bold tracking-wider bg-red-500 text-white px-1.5 py-0.5 rounded animate-pulse">
                관리중
              </span>
            )}
          </div>
        ...
        <span className="text-[10px] font-semibold tracking-wider uppercase text-gray-400">
          {therapist.slots.length}건
        </span>
      </div>

      {/* Slots */}
      <div
        className={`flex-1 p-2 flex flex-col gap-2 overflow-y-auto transition-colors ${
          slotOver ? 'bg-emerald-900/20' : ''
        }`}
      >
        ...
        {Array.from({ length: emptyCount }).map((_, i) => (
          <button
            key={`empty-${i}`}
            onClick={onAddSlot}
            className="w-full h-[80px] border border-dashed border-slate-700/60 hover:border-emerald-600/60 hover:bg-emerald-900/10 rounded-lg flex flex-col items-center justify-center gap-1 transition-all duration-150"
          >
            <span className="text-[#888] text-xl leading-none">+</span>
            <span className="text-[#888] text-[10px] font-semibold tracking-wider">추가</span>
          </button>
        ))}
      </div>
    </div>
