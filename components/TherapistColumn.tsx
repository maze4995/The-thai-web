  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="px-3 py-2.5 bg-slate-50 dark:bg-[#1a2035] border-b border-slate-200 dark:border-slate-700/40 flex items-center justify-between cursor-grab active:cursor-grabbing select-none"
      >
        <div className="flex items-center gap-1.5">
          <span className="font-bold text-slate-900 dark:text-slate-100 text-sm">{therapist.name}</span>
          {isServing && (
            <span className="text-[10px] font-bold tracking-wider bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded animate-pulse">
              관리중
            </span>
          )}
        </div>
      ...
      >
        ...
        {Array.from({ length: emptyCount }).map((_, i) => (
          <button
            key={`empty-${i}`}
            onClick={onAddSlot}
            className="w-full h-[80px] border border-dashed border-slate-300 dark:border-slate-700/60 hover:border-emerald-600/60 hover:bg-emerald-900/10 rounded-lg flex items-center justify-center text-slate-400 dark:text-slate-600 hover:text-emerald-500 transition-all duration-150 text-sm"
          >
            + 추가
          </button>
        ))}
