...
export default function Sidebar({ storeName, onSignOut }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`py-6 ${collapsed ? 'px-0 flex justify-center' : 'px-5'}`}>
        {collapsed ? (
          <span className="text-[#D4A574] text-lg font-bold">T</span>
        ) : (
          <>
            <h1 className="text-white text-xl font-bold tracking-wide">The Thai</h1>
            <p className="text-gray-400 text-xs mt-1">매장 관리 시스템</p>
          </>
        )}
      </div>
      ...
      {/* Bottom area */}
      <div className={`py-4 border-t border-white/10 ${collapsed ? 'px-0 flex flex-col items-center gap-2' : 'px-5'}`}>
        {!collapsed && storeName && (
